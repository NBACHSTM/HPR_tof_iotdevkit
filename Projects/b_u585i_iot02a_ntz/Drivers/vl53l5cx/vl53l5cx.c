...
static int32_t vl53l5cx_get_result(VL53L5CX_Object_t *pObj);
...
int32_t VL53L5CX_ConfigProfile(VL53L5CX_Object_t *pObj, VL53L5CX_ProfileConfig_t *pConfig)
{
  ...
  {
    pObj->IsAmbientEnabled = (uint8_t)(pConfig->EnableAmbient == 0U) ? 0U : 1U;
    pObj->IsSignalEnabled = (uint8_t)(pConfig->EnableSignal == 0U) ? 0U : 1U;
...
/**
  ...
  * @retval VL53L5CX status
  */
int32_t VL53L5CX_GetDistance(VL53L5CX_Object_t *pObj)
{
  int32_t ret;
  ...
  if (ret == VL53L5CX_OK)
  {
    ret = vl53l5cx_get_result(pObj);
  //  ret = vl53l5cx_get_ranging_data(pObj, &(App_Config->RangingData));
  }
...
static int32_t vl53l5cx_poll_for_measurement(VL53L5CX_Object_t *pObj, uint32_t Timeout)
{
  ...
  else
  {
    ...
    } while (((uint32_t)(pObj->IO.GetTick()) - TickStart) < Timeout);
...
static int32_t vl53l5cx_get_result(VL53L5CX_Object_t *pObj)
{
  ...
  else if (vl53l5cx_get_ranging_data(&pObj->Dev) != VL53L5CX_STATUS_OK)
  {
    ret = VL53L5CX_ERROR;
    App_Config.new_data_received = 0;
  }
  else
  {
	App_Config.new_data_received = 1;
